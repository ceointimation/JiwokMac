...
-(NSMutableArray*)getArtists:(NSString *)filePath{
	DUBUG_LOG(@"Now you are in getArtists method in JiwokLastfmClient class");
	//	NSString * toolPath;	
	...
	DUBUG_LOG(@"Now you are completed getArtists method in JiwokLastfmClient class");	
}



-(NSMutableArray *)GetArtistTopTag:(NSString*)artist{
	DUBUG_LOG(@"Now you are in GetArtistTopTag method in JiwokLastfmClient class");
	...
	NSDictionary *urlDict = [NSDictionary dictionaryWithObjectsAndKeys:artist, @"artist", _LASTFM_API_KEY_, @"api_key", nil, nil];
     NSLog(@"urlDict NSDictionary==%@",urlDict);
	...
	NSDictionary *statuses = [parser objectWithString:json_string error:nil];
	 NSLog(@"statuses NSDictionary==%@",statuses);
	[parser release];	
...
//	[[NSNotificationCenter defaultCenter] postNotificationName:@"GetArtistTopTagFinished" object:topTags];	
    ...
    DUBUG_LOG(@"Now you are completed GetArtistTopTag method in JiwokLastfmClient class");
}


- (void)GetArtistCallBack:(NSString *)identifier data:(id)data {		
	//NSLog(@"Now you are in GetArtistCallBack method in JiwokLastfmClient class");
	DUBUG_LOG(@"GetArtistCallBack");
	//NSLog(@"Now you are completed GetArtistCallBack method in JiwokLastfmClient class");
}


-(NSMutableArray *)GetAlbumTopTag:(NSString*)artist:(NSString*)album
{	
	DUBUG_LOG(@"Now you are in GetAlbumTopTag method in JiwokLastfmClient class");
	...
	//[[NSNotificationCenter defaultCenter] postNotificationName:@"GetAlbumTopTagFinished" object:topTags];	
	 NSLog(@"topTags NSMutableArray==%@",topTags);
	return [topTags autorelease];
		
	}
	DUBUG_LOG(@"Now you are completed GetAlbumTopTag method in JiwokLastfmClient class");
}


- (void)GetAlbumCallBack:(NSString *)identifier data:(id)data {		
	NSLog(@"Now you are in GetAlbumCallBack method in JiwokLastfmClient class");
		DUBUG_LOG(@"GetAlbumCallBack");
	NSLog(@"Now you are completed GetAlbumCallBack method in JiwokLastfmClient class");
